"""
Construct Temporal Graph from Transactions.
Author: Liu Yang
Date: 2019-4-23
"""
...
def add_to_graph(file_path, graph, graph_type, start_block, end_block):
    ...
    Note:
        When using Networkx, the edge attribute 'weight' should be numeric. 
        (https://networkx.github.io/documentation/stable/tutorial.html#edge-attributes) 
    """
    ...
    for row in data_reader:
        if int(row[1]) in range(start_block, end_block):
            ...
            if graph_type == 'CIG':
                ...
                value = float(row[6])
            G.add_edge(fromaddr, toaddr, weight=value, time=timestamp)
    data_file.close()
...
if __name__ == "__main__":
    ...
    nx.write_edgelist(G_int, args.output, delimiter=' ', data=['time'])
